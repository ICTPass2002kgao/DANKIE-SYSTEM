...
class _AddCareerOpportunitiesState extends State<AddCareerOpportunities>
  ...
  Future<List<dynamic>> _fetchOpportunities() async {
    final url = Uri.parse('${Api().BACKEND_BASE_URL_DEBUG}/careers/');
  ...
  void _populateFormForEditing(Map<String, dynamic> data) {
    Map<String, dynamic> details = data['details'] ?? {};

    setState(() {
      _editingId = data['id']?.toString() ?? data['uid']?.toString();
      ...
      if (data['expiryDate'] != null) {
        _expiryDate = DateTime.tryParse(data['expiryDate'].toString());
      } else {
        _expiryDate = null;
      }
    ...
    try {
      ...
      if (_editingId != null) {
        ...
        final updateUrl = Uri.parse(
          '${Api().BACKEND_BASE_URL_DEBUG}/careers/$_editingId/',
        );
        final response = await http.put(
          updateUrl,
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode(mainData),
        );
        ...
        // CREATE New API Call
        final createUrl = Uri.parse('${Api().BACKEND_BASE_URL_DEBUG}/careers/');
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          throw Exception("Failed to publish: ${response.body}");
        }
      }
  ...
  Future<void> _deleteOpportunity(String docId) async {
    try {
      final deleteUrl = Uri.parse(
        '${Api().BACKEND_BASE_URL_DEBUG}/careers/$docId/',
      );
  ...
  Widget _buildNeumorphicTextField({
    ...
    // Inset (Sunken) Neumorphic Container for Inputs
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor:
          theme.scaffoldBackgroundColor, // Ensure base color matches
      body: Column(
        children: [
          // Neumorphic Tab Bar Container
          NeumorphicContainer(
            borderRadius: 0, // Flat bottom/top for tabs usually
          ...
          if (_editingId != null)
            Container(
              ...
              child: Row(
                ...
                children: [
                  Text("Editing Mode", style: TextStyle(color: Colors.blue)),
                  IconButton(
    ...
    return _isLoading
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                // 1. Image Banner (Neumorphic Inset)
                GestureDetector(
                  onTap: _pickImage,
                ...
                // Submit Button (Neumorphic Popped)
                GestureDetector(
                  onTap: _submitData,
                  child: NeumorphicContainer(
                    isPressed: false,
                    ...
                    // For the main action, we can use primary color background logic if desired,
                    // but strict neomorphism uses same color. Let's use primary text/icon.
                    child: Center(
                      child: Text(
                        _editingId != null
                            ? "Update Opportunity"
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            String docId =
                data['id']?.toString() ?? data['uid']?.toString() ?? '';
      ...
      children: [
        ...
        Row(
          children: [
            ...
            GestureDetector(
              onTap: () => _addItemToList(controller, targetList),
              child: NeumorphicContainer(
                isPressed: false,
                borderRadius: 8,
        ...
        if (targetList.isNotEmpty)
          NeumorphicContainer(
            isPressed: true, // Inset background for list
            borderRadius: 8,
