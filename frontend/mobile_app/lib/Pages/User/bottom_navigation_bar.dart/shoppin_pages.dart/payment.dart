  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Container(
        ...
        child: Column(
          children: [
            Api().buildAppBar(context, "Login Required")!,
...
class _PaymentGatewayPageState extends State<PaymentGatewayPage> {
  ...
  Future<void> _fetchUserAddressFromDjango() async {
    ...
    // Do not fetch address if user is anonymous/guest
  ...
  void _showLoginPrompt() {
    // Push the InlineLoginForm as a full page so we can await its return.
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => Scaffold(
          ...
          body: Center(
            child: ConstrainedBox(
              ...
              child: InlineLoginForm(
                onSuccess: () {
                  Navigator.pop(context); // Pop back to the cart page
                  Api().showMessage(
                    context,
      ...
      // Refresh the UI and load the address immediately after returning
      setState(() {});
      _fetchUserAddressFromDjango();
    });
  }
  ...
  Future<void> _payWithPaystack() async {
    ...
    // Check if the user is null OR logged in as an anonymous guest
